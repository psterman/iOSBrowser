...
class AIChatManager: ObservableObject {
    ...
    func getRecentSessions(for aiService: String, limit: Int = 5) -> [AIChatSession] {
        return Array(chatSessions.filter { $0.aiService == aiService }.prefix(limit))
    }
...
struct BrowserView: View {
    ...
    // AI对话管理器
    @StateObject private var aiChatManager = AIChatManager.shared
    ...
    var body: some View {
        ...
        .sheet(isPresented: $aiChatManager.isChatActive) {
            AIChatView()
        }
                    ...
                    AIDrawerView(
                        ...
                        onAISelected: { service in
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                                ...
                                // 激活真正的AI对话
                                aiChatManager.startNewChat(with: service)
                            }
                        }
                    )
    ...
    private func toggleFavorite() {
        ...
        if favoritePages.contains(currentURL) {
            ...
            showToast("已取消收藏", type: .info)
        } else {
            favoritePages.insert(currentURL)
            showToast("已收藏", type: .success)
        }
        saveFavorites()
    }
}
...
struct BrowserAIChatView: View {
    ...
    // 支持API的AI列表
    private let aiList = [
        ("deepseek", "DeepSeek", "brain.head.profile", Color.purple, "https://api.deepseek.com"),
        ("qwen", "通义千问", "cloud.fill", Color.cyan, "https://dashscope.aliyuncs.com"),
        ("chatglm", "智谱清言", "lightbulb.fill", Color.yellow, "https://open.bigmodel.cn"),
        ("moonshot", "Kimi", "moon.stars.fill", Color.orange, "https://api.moonshot.cn"),
        ("claude", "Claude", "c.circle.fill", Color.blue, "https://api.anthropic.com"),
        ("gpt", "ChatGPT", "g.circle.fill", Color.green, "https://api.openai.com")
    ]
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingAPIConfig) {
                BrowserAPIConfigView()
            }
            .toast(isPresented: $showingToast) {
                ToastView(message: toastMessage, type: .info)
            }
        }
        .onAppear {
    ...
    private func sendMessage() {
        ...
        // 检查API配置
        let apiKey = UserDefaults.standard.string(forKey: "api_key_\(selectedAI)")
        if apiKey == nil || apiKey!.isEmpty {
            showToast("请先配置\(getAIName(selectedAI))的API密钥")
            return
        }
        
        // 调用真实API
        isLoading = true
        callAIAPI(message: trimmedMessage, aiId: selectedAI) { result in
            ...
            messages.append(aiMessage)
                case .failure(let error):
                    showToast("API调用失败: \(error.localizedDescription)")
                }
            }
        }
    }
    
    private func callAIAPI(message: String, aiId: String, completion: @escaping (Result<String, Error>) -> Void) {
        guard let apiKey = UserDefaults.standard.string(forKey: "api_key_\(aiId)"), !apiKey.isEmpty else {
            completion(.failure(NSError(domain: "API", code: 1, userInfo: [NSLocalizedDescriptionKey: "API密钥未配置"])))
            return
        }
        ...
        switch aiId {
        case "deepseek":
            callDeepSeekAPI(message: message, apiKey: apiKey, completion: completion)
        case "qwen":
            callQwenAPI(message: message, apiKey: apiKey, completion: completion)
        case "chatglm":
            callChatGLMAPI(message: message, apiKey: apiKey, completion: completion)
        default:
            // 模拟API调用
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                completion(.success("这是来自\(getAIName(aiId))的回复：\n\n\(message)"))
            }
        }
    }
    ...
    private func callDeepSeekAPI(message: String, apiKey: String, completion: @escaping (Result<String, Error>) -> Void) {
        guard let url = URL(string: "https://api.deepseek.com/v1/chat/completions") else {
            completion(.failure(NSError(domain: "API", code: 2, userInfo: [NSLocalizedDescriptionKey: "无效的API地址"])))
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard let data = data else {
                completion(.failure(NSError(domain: "API", code: 3, userInfo: [NSLocalizedDescriptionKey: "无响应数据"])))
                return
            }
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   ...
                   let content = message["content"] as? String {
                    ...
                    completion(.failure(NSError(domain: "API", code: 4, userInfo: [NSLocalizedDescriptionKey: "响应格式错误"])))
                }
            } catch {
                completion(.failure(error))
            }
    ...
    private func callQwenAPI(message: String, apiKey: String, completion: @escaping (Result<String, Error>) -> Void) {
        // 通义千问API调用实现
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            completion(.success("通义千问回复：\n\n\(message)"))
        }
    }
    
    private func callChatGLMAPI(message: String, apiKey: String, completion: @escaping (Result<String, Error>) -> Void) {
        // 智谱清言API调用实现
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            completion(.success("智谱清言回复：\n\n\(message)"))
        }
    }
...
struct ToastView: View {
    let message: String
    let type: BrowserView.ToastType
...
struct BrowserAPIConfigView: View {
        ...
        NavigationView {
            List {
                Section(header: Text("AI服务配置")) {
                    ForEach(aiServices, id: \.0) { service in
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                Image(systemName: service.2)
                                    .foregroundColor(service.3)
                                    .font(.system(size: 20))
                ...
                Section(header: Text("说明"), footer: Text("API密钥将安全存储在本地，不会上传到服务器")) {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("• DeepSeek: 访问 https://platform.deepseek.com 获取API密钥")
                        Text("• 通义千问: 访问 https://dashscope.console.aliyun.com 获取API密钥")
                        Text("• 智谱清言: 访问 https://open.bigmodel.cn 获取API密钥")
                        Text("• 其他AI服务请访问对应官网获取API密钥")
                    }
                    .font(.system(size: 12))
                    .foregroundColor(.secondary)
                }
    ...
    private func loadAPIKeys() {
        for service in aiServices {
            if let key = UserDefaults.standard.string(forKey: "api_key_\(service.0)") {
                apiKeys[service.0] = key
            }
        }
    }
    
    private func saveAPIKeys() {
        for (serviceId, key) in apiKeys {
...
struct AIDrawerView: View {
    ...
    @ObservedObject private var aiChatManager = AIChatManager.shared
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                LazyVStack(spacing: 0) {
                    ForEach(aiServices, id: \.id) { service in
                        AIDrawerItem(
                            service: service,
                            sessionCount: aiChatManager.getSessionCount(for: service.id),
                            recentSessions: aiChatManager.getRecentSessions(for: service.id, limit: 3),
                            onTap: {
                                onAISelected(service)
                            },
                            onSessionTap: { session in
                                aiChatManager.continueChat(session: session)
                                isPresented = false
                            }
                        )
                    }
...
// MARK: - 抽屉式AI服务项目
struct AIDrawerItem: View {
    let service: AIService
    let sessionCount: Int
    let recentSessions: [AIChatSession]
    let onTap: () -> Void
    let onSessionTap: (AIChatSession) -> Void
