...
struct WidgetConfigView: View {
    ...
    private func resetToDefaults() {
        ...
        dataSyncCenter.selectedAIAssistants = ["openai", "claude"]
...
struct ContentView: View {
    ...
    private func clearAIData(for contactId: String) {
        print("🗑️ 开始清理AI数据: \(contactId)")
        
        let alert = UIAlertController(
            title: "确认清理AI数据",
            message: "⚠️ 此操作将永久删除以下数据：\n\n• \(contactId)的所有聊天记录\n• \(contactId)的API密钥\n• \(contactId)的配置信息\n\n清理后，该AI将被禁用，需要重新配置API密钥才能使用。\n\n确定要继续吗？",
        ...
        alert.addAction(UIAlertAction(title: "确认清理", style: .destructive) { _ in
            self.performAICleanup(for: contactId)
        })
    ...
    private func performAICleanup(for contactId: String) {
        print("🧹 执行AI数据清理: \(contactId)")
        
        // 清理聊天记录
        let historyKey = "chat_history_\(contactId)"
        UserDefaults.standard.removeObject(forKey: historyKey)
        
        // 清理聊天会话
        let sessionsKey = "AIChatSessions"
        if let existingData = UserDefaults.standard.data(forKey: sessionsKey) {
            do {
                if var sessions = try JSONSerialization.jsonObject(with: existingData) as? [[String: Any]] {
                    // 过滤掉当前AI的会话
                    sessions = sessions.filter { session in
                        if let aiId = session["aiId"] as? String {
                            return aiId != contactId
                        }
                        return true
                    }
                    
                    // 保存清理后的会话
                    if let newData = try? JSONSerialization.data(withJSONObject: sessions) {
                        UserDefaults.standard.set(newData, forKey: sessionsKey)
                    }
                }
            } catch {
                print("❌ 清理聊天会话失败: \(error)")
            }
        }
        
        // 清理API密钥
        if contactId == "deepseek" {
            UserDefaults.standard.removeObject(forKey: "deepseek_API_KEY")
        } else {
            UserDefaults.standard.removeObject(forKey: "\(contactId)_API_KEY")
        }
        
        // 清理网络超时配置
        UserDefaults.standard.removeObject(forKey: "network_timeout")
        UserDefaults.standard.removeObject(forKey: "api_timeout")
        UserDefaults.standard.removeObject(forKey: "request_timeout")
        
        // 禁用AI联系人
        UserDefaults.standard.set(false, forKey: "contact_enabled_\(contactId)")
        UserDefaults.standard.set(false, forKey: "contact_pinned_\(contactId)")
        UserDefaults.standard.set(true, forKey: "contact_hidden_\(contactId)")
        
        // 显示清理完成提示
        let successAlert = UIAlertController(
            title: "清理完成",
            message: "✅ \(contactId)的数据已成功清理\n\n• 聊天记录已删除\n• API密钥已清除\n• AI已被禁用\n\n如需重新使用，请重新配置API密钥。",
            preferredStyle: .alert
        )
        
        successAlert.addAction(UIAlertAction(title: "重新配置", style: .default) { _ in
            self.showAPIConfiguration(for: contactId)
        })
        ...
        print("✅ AI数据清理完成: \(contactId)")
    }
    
    private func showAPIConfiguration(for contactId: String) {
        print("🔑 显示API配置界面: \(contactId)")
        
        let alert = UIAlertController(
            title: "配置\(contactId) API密钥",
            message: "请输入您的API密钥以重新激活\(contactId)：",
            preferredStyle: .alert
        )
        
        alert.addTextField { textField in
            textField.placeholder = "请输入API密钥"
            textField.text = ""
        }
        
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        
        alert.addAction(UIAlertAction(title: "激活", style: .default) { _ in
            if let apiKey = alert.textFields?.first?.text, !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                self.activateAI(with: apiKey, for: contactId)
            } else {
                // 显示错误提示
    ...
    private func activateAI(with apiKey: String, for contactId: String) {
        print("🔑 激活AI: \(contactId)")
        
        // 保存API密钥
        if contactId == "deepseek" {
            UserDefaults.standard.set(apiKey, forKey: "deepseek_API_KEY")
        } else {
            UserDefaults.standard.set(apiKey, forKey: "\(contactId)_API_KEY")
        }
        
        // 设置网络超时配置
        UserDefaults.standard.set(30.0, forKey: "network_timeout")
        UserDefaults.standard.set(30.0, forKey: "api_timeout")
        UserDefaults.standard.set(30.0, forKey: "request_timeout")
        
        // 启用AI联系人
        UserDefaults.standard.set(true, forKey: "contact_enabled_\(contactId)")
        UserDefaults.standard.set(false, forKey: "contact_pinned_\(contactId)")
        UserDefaults.standard.set(false, forKey: "contact_hidden_\(contactId)")
        
        // 显示激活成功提示
        let successAlert = UIAlertController(
            title: "激活成功",
            message: "✅ \(contactId)已成功激活\n\n• API密钥已配置\n• 网络超时已设置\n• AI已启用\n\n现在可以正常使用\(contactId)了！",
            preferredStyle: .alert
        )
...
struct ChatView: View {
    ...
    private func callOpenAIAPIDirectly(message: String, apiKey: String) {
        ...
        request.timeoutInterval = 15.0 // 添加超时设置
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                self.parseOpenAIAPIResponse(data: data)
            }
        }.resume()
    }

    private func parseOpenAIAPIResponse(data: Data) {
        do {
    ...
    private func checkNetworkStatus() {
        ...
        let task = URLSession.shared.dataTask(with: url) { _, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("⚠️ 网络连接检查失败: \(error.localizedDescription)")
                    // 显示网络状态提示
                    self.showNetworkStatusWarning()
                } else if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        print("✅ 网络连接正常")
                    } else {
                        print("⚠️ 网络响应异常: \(httpResponse.statusCode)")
                        self.showNetworkStatusWarning()
                    }
                }
            }
...
struct SimpleAIChatView: View {
    ...
    private func performAICleanup(for contactId: String) {
        ...
        if contactId == "deepseek" {
            ...
            UserDefaults.standard.set("", forKey: "\(contactId)_API_KEY")
        }
    ...
    private func startAIChatTest(for contactId: String) {
        print("🔧 开始测试AI对话初始化: \(contactId)")
        
        // 自动配置DeepSeek API（如果需要）
        if contactId == "deepseek" {
        ...
        // 测试步骤1: 基本环境检查
        currentStep = 1
        testResults.append("📋 步骤1: 测试基本环境检查")
        
        do {
            // 检查UserDefaults是否可用
            let testValue = "test_\(Date().timeIntervalSince1970)"
            UserDefaults.standard.set(testValue, forKey: "test_key")
            let retrievedValue = UserDefaults.standard.string(forKey: "test_key")
            UserDefaults.standard.removeObject(forKey: "test_key")
            
            if retrievedValue == testValue {
                testResults.append("✅ 步骤1成功: 基本环境检查通过")
            } else {
                testResults.append("❌ 步骤1失败: UserDefaults访问异常")
                showTestResults(testResults, contactId: contactId)
                return
            }
        } catch {
            testResults.append("❌ 步骤1失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        ...
        // 测试步骤2: API密钥检查
        currentStep = 2
        testResults.append("📋 步骤2: 测试API密钥检查")
        
        do {
            // 特殊处理DeepSeek的API密钥检查
            let apiKey: String?
            if contactId == "deepseek" {
                apiKey = UserDefaults.standard.string(forKey: "deepseek_API_KEY")
            } else {
                apiKey = UserDefaults.standard.string(forKey: "\(contactId)_API_KEY")
            }
            
            if let key = apiKey, !key.isEmpty {
                testResults.append("✅ 步骤2成功: API密钥已配置")
                testResults.append("   - 密钥长度: \(key.count) 字符")
                testResults.append("   - 密钥格式: \(key.hasPrefix("sk-") ? "正确" : "可能不正确")")
            } else {
                testResults.append("⚠️ 步骤2警告: API密钥未配置")
                if contactId == "deepseek" {
                    testResults.append("   - 建议: 请配置DeepSeek API密钥")
                }
            }
        } catch {
            testResults.append("❌ 步骤2失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        ...
        // 测试步骤3: 联系人状态检查
        currentStep = 3
        testResults.append("📋 步骤3: 测试联系人状态检查")
        
        do {
            let isEnabled = UserDefaults.standard.bool(forKey: "contact_enabled_\(contactId)")
            let isPinned = UserDefaults.standard.bool(forKey: "contact_pinned_\(contactId)")
            let isHidden = UserDefaults.standard.bool(forKey: "contact_hidden_\(contactId)")
            
            testResults.append("✅ 步骤3成功: 联系人状态检查完成")
            testResults.append("   - 启用状态: \(isEnabled ? "是" : "否")")
            testResults.append("   - 置顶状态: \(isPinned ? "是" : "否")")
            testResults.append("   - 隐藏状态: \(isHidden ? "否" : "是")")
        } catch {
            testResults.append("❌ 步骤3失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        ...
        // 测试步骤4: 聊天历史检查
        currentStep = 4
        testResults.append("📋 步骤4: 测试聊天历史检查")
        
        do {
            let historyKey = "chat_history_\(contactId)"
            if let data = UserDefaults.standard.data(forKey: historyKey) {
                let messageCount = data.count
                testResults.append("✅ 步骤4成功: 聊天历史检查完成")
                testResults.append("   - 历史数据大小: \(messageCount) 字节")
            } else {
                testResults.append("✅ 步骤4成功: 聊天历史检查完成")
                testResults.append("   - 暂无聊天历史")
            }
        } catch {
            testResults.append("❌ 步骤4失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        ...
        // 测试步骤5: 界面状态检查
        currentStep = 5
        testResults.append("📋 步骤5: 测试界面状态检查")
        
        do {
            let currentContact = contacts.first { $0.id == contactId }
            if let contact = currentContact {
                testResults.append("✅ 步骤5成功: 界面状态检查完成")
                testResults.append("   - 联系人名称: \(contact.name)")
                testResults.append("   - 联系人描述: \(contact.description)")
                testResults.append("   - 需要API密钥: \(contact.requiresApiKey ? "是" : "否")")
            } else {
                testResults.append("❌ 步骤5失败: 找不到联系人")
                showTestResults(testResults, contactId: contactId)
                return
            }
        } catch {
            testResults.append("❌ 步骤5失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        ...
        // 测试步骤6: 系统资源检查
        currentStep = 6
        testResults.append("📋 步骤6: 测试系统资源检查")
        
        do {
            let memoryUsage = getMemoryUsage()
            let diskSpace = getDiskSpace()
            
            testResults.append("✅ 步骤6成功: 系统资源检查完成")
            testResults.append("   - 内存使用: \(memoryUsage) MB")
            testResults.append("   - 磁盘空间: \(diskSpace) GB")
            
            // 检查内存使用是否异常
            if let memoryValue = Double(memoryUsage), memoryValue > 500 {
                testResults.append("⚠️ 步骤6警告: 内存使用过高(\(memoryUsage)MB)，可能导致性能问题")
            }
        } catch {
            testResults.append("❌ 步骤6失败: \(error.localizedDescription)")
            showTestResults(testResults, contactId: contactId)
            return
        }
        
        // 完成测试
        testResults.append("🎉 AI对话初始化测试完成！")
        
        let successCount = testResults.filter { $0.contains("✅") }.count
        let failCount = testResults.filter { $0.contains("❌") }.count
        let warningCount = testResults.filter { $0.contains("⚠️") }.count
        
        testResults.append("📊 测试结果: 成功\(successCount)步，失败\(failCount)步，警告\(warningCount)步")
        
        // 分析可能的死机原因
        if failCount > 0 {
            testResults.append("🚨 发现严重问题！这些步骤失败可能是死机原因")
        } else if warningCount > 0 {
            testResults.append("⚠️ 发现多个警告，这些可能是死机的潜在原因：")
            
            // 分析具体的警告信息
            let warnings = testResults.filter { $0.contains("⚠️") }
            for warning in warnings {
                if warning.contains("API密钥未配置") {
                    testResults.append("   • API密钥未配置 - 可能导致网络请求失败")
                } else if warning.contains("内存使用过高") {
                    testResults.append("   • 内存使用过高 - 可能导致应用崩溃")
                }
            }
        } else {
            testResults.append("✅ 所有测试通过，理论上不应该死机")
        }
        
        showTestResults(testResults, contactId: contactId)
    }
    
    private func showTestResults(_ results: [String], contactId: String) {
        let alert = UIAlertController(
            title: "AI对话初始化测试结果",
        ...
        // 添加一键修复选项（仅对DeepSeek显示）
        if contactId == "deepseek" {
            alert.addAction(UIAlertAction(title: "一键修复配置", style: .destructive) { _ in
                self.configureDeepSeekAPI()
            })
        }
        
        // 添加深入测试选项
        alert.addAction(UIAlertAction(title: "深入测试", style: .default) { _ in
            self.startDeepAIChatTest(for: contactId)
        })
    ...
    private func startDeepAIChatTest(for contactId: String) {
        print("🔧 开始深入AI对话界面测试: \(contactId)")
        ...
        // 测试步骤1: 模拟创建AI对话视图
        currentStep = 1
        testResults.append("📋 步骤1: 模拟创建AI对话视图")
        
        do {
            // 模拟创建AIChatView的关键组件
            let testMessageText = ""
            let testShowingSessionList = false
            let testShowingAPIConfig = false
            
            // 检查这些变量是否能正常创建
            if testMessageText.isEmpty && !testShowingSessionList && !testShowingAPIConfig {
                testResults.append("✅ 步骤1成功: 基础状态变量创建正常")
            } else {
                testResults.append("❌ 步骤1失败: 基础状态变量创建异常")
                showDeepTestResults(testResults, contactId: contactId)
                return
            }
        } catch {
            testResults.append("❌ 步骤1失败: \(error.localizedDescription)")
        ...
        // 测试步骤2: 模拟AIChatManager初始化
        currentStep = 2
        testResults.append("📋 步骤2: 模拟AIChatManager初始化")
        
        do {
            // 尝试访问UserDefaults中的聊天数据
            let chatSessionsKey = "AIChatSessions"
            if let data = UserDefaults.standard.data(forKey: chatSessionsKey) {
                let dataSize = data.count
                testResults.append("✅ 步骤2成功: 聊天会话数据检查完成")
                testResults.append("   - 数据大小: \(dataSize) 字节")
                
                // 尝试解码数据
                if dataSize > 0 {
                    do {
                        let _ = try JSONSerialization.jsonObject(with: data)
                        testResults.append("   - JSON解码: 成功")
                    } catch {
                        testResults.append("   - JSON解码: 失败 - \(error.localizedDescription)")
                        testResults.append("⚠️ 警告: 聊天数据可能损坏，这可能是死机原因")
                    }
                }
            } else {
                testResults.append("✅ 步骤2成功: 暂无聊天会话数据")
            }
        } catch {
            testResults.append("❌ 步骤2失败: \(error.localizedDescription)")
            showDeepTestResults(testResults, contactId: contactId)
        ...
        // 测试步骤3: 模拟消息列表渲染
        currentStep = 3
        testResults.append("📋 步骤3: 模拟消息列表渲染")
        ...
        do {
            ...
            if let data = UserDefaults.standard.data(forKey: historyKey) {
                let messageCount = data.count
                if messageCount > 100000 { // 100KB
                    testResults.append("⚠️ 步骤3警告: 消息数据过大(\(messageCount)字节)，可能导致渲染卡死")
                } else {
                    testResults.append("✅ 步骤3成功: 消息数据大小正常")
                }
                
                // 尝试解析消息数据
                if let messages = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]] {
                    let actualMessageCount = messages.count
                    testResults.append("   - 实际消息数量: \(actualMessageCount)")
                    
                    if actualMessageCount > 1000 {
                        testResults.append("⚠️ 警告: 消息数量过多(\(actualMessageCount)条)，可能导致界面卡死")
                    }
                }
            } else {
                testResults.append("✅ 步骤3成功: 暂无消息数据")
            }
        } catch {
            testResults.append("❌ 步骤3失败: \(error.localizedDescription)")
            showDeepTestResults(testResults, contactId: contactId)
        ...
        // 测试步骤4: 模拟配置数据检查
        currentStep = 4
        testResults.append("📋 步骤4: 模拟配置数据检查")
        ...
        do {
            ...
            for key in configKeys {
                if let data = UserDefaults.standard.data(forKey: key) {
                    totalConfigSize += data.count
                }
            }
            
            if totalConfigSize > 10000 { // 10KB
                testResults.append("⚠️ 步骤4警告: 配置数据过大(\(totalConfigSize)字节)，可能影响性能")
            } else {
                testResults.append("✅ 步骤4成功: 配置数据大小正常")
            }
        } catch {
            testResults.append("❌ 步骤4失败: \(error.localizedDescription)")
            showDeepTestResults(testResults, contactId: contactId)
        ...
        // 测试步骤5: 模拟异步操作和网络请求
        currentStep = 5
        testResults.append("📋 步骤5: 模拟异步操作和网络请求")
        ...
        do {
            ...
            if hasTimeoutConfig {
                testResults.append("✅ 步骤5成功: 网络超时配置已设置")
            } else {
                testResults.append("⚠️ 步骤5警告: 未设置网络超时，可能导致请求无限等待")
            }
            ...
            if let key = apiKey {
                if key.count < 10 {
                    testResults.append("⚠️ 警告: API密钥格式可能不正确")
                } else {
                    testResults.append("✅ API密钥格式检查正常")
                    testResults.append("   - 密钥长度: \(key.count) 字符")
                    if contactId == "deepseek" {
                        testResults.append("   - 密钥格式: \(key.hasPrefix("sk-") ? "正确" : "可能不正确")")
                    }
                ...
                testResults.append("⚠️ 警告: API密钥未配置")
            }
        } catch {
            testResults.append("❌ 步骤5失败: \(error.localizedDescription)")
            showDeepTestResults(testResults, contactId: contactId)
        ...
        // 测试步骤6: 模拟内存和性能检查
        currentStep = 6
        testResults.append("📋 步骤6: 模拟内存和性能检查")
        ...
        do {
            ...
            if let memoryValue = Double(memoryUsage), memoryValue > 500 {
                ...
                testResults.append("✅ 步骤6成功: 内存使用正常")
            }
            ...
            var leakWarnings = 0
            for indicator in leakIndicators {
                if UserDefaults.standard.object(forKey: indicator) != nil {
                    leakWarnings += 1
                }
            }
            
            if leakWarnings > 0 {
                testResults.append("⚠️ 警告: 发现\(leakWarnings)个可能的内存泄漏指标")
            }
        } catch {
            testResults.append("❌ 步骤6失败: \(error.localizedDescription)")
            showDeepTestResults(testResults, contactId: contactId)
        ...
        // 完成测试
        testResults.append("🎉 深入AI对话界面测试完成！")
        ...
        if failCount > 0 {
            ...
            let warnings = testResults.filter { $0.contains("⚠️") }
            for warning in warnings {
                if warning.contains("聊天数据损坏") {
                    testResults.append("   • 聊天数据损坏 - 可能导致解析失败")
                } else if warning.contains("消息数量过多") {
                    testResults.append("   • 消息数量过多 - 可能导致渲染卡死")
                } else if warning.contains("配置项数据过大") {
                    testResults.append("   • 配置项数据过大 - 可能影响性能")
                } else if warning.contains("网络超时缺失") {
                    testResults.append("   • 网络超时缺失 - 可能导致请求无限等待")
                } else if warning.contains("内存使用过高") {
                    testResults.append("   • 内存使用过高 - 可能导致应用崩溃")
                } else if warning.contains("内存泄漏指标") {
                    testResults.append("   • 内存泄漏指标 - 可能导致性能下降")
                }
            }
        } else {
            testResults.append("✅ 所有测试通过，理论上不应该死机")
    ...
    private func showDeepTestResults(_ results: [String], contactId: String) {
        let alert = UIAlertController(
            title: "深入AI对话界面测试结果",
            message: results.joined(separator: "\n"),
            preferredStyle: .alert
        ...
        // 添加尝试进入对话选项
        alert.addAction(UIAlertAction(title: "尝试进入对话", style: .default) { _ in
            self.attemptEnterChat(for: contactId)
        })
        
        // 添加查看详细日志选项
        alert.addAction(UIAlertAction(title: "查看详细日志", style: .default) { _ in
            self.showDetailedLogs(for: contactId)
        })
    ...
    private func attemptEnterChat(for contactId: String) {
        let alert = UIAlertController(
            title: "尝试进入对话",
            message: "即将尝试进入\(contactId)的对话界面。如果发生死机，请强制重启应用。\n\n确定要继续吗？",
            preferredStyle: .alert
        )
        
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        
        alert.addAction(UIAlertAction(title: "继续", style: .destructive) { _ in
            // 尝试直接进入对话
            if let contact = self.contacts.first(where: { $0.id == contactId }) {
                self.currentContact = contact
                self.showingDirectChat = true
            }
        })
    ...
    private func showDetailedLogs(for contactId: String) {
        var logInfo: [String] = []
        
        // 系统信息
        logInfo.append("📱 系统信息:")
        logInfo.append("• 设备: \(UIDevice.current.model)")
        logInfo.append("• 系统: iOS \(UIDevice.current.systemVersion)")
        logInfo.append("• 应用版本: 1.0")
        logInfo.append("")
        
        // 配置信息
        logInfo.append("⚙️ 配置信息:")
        let deepseekKey = UserDefaults.standard.string(forKey: "deepseek_API_KEY")
        logInfo.append("• deepseek_API_KEY: \(deepseekKey != nil ? "已设置" : "未设置")")
        let autoTest = UserDefaults.standard.bool(forKey: "AIChat_AutoTestOnAppear")
        logInfo.append("• AIChat_AutoTestOnAppear: \(autoTest ? "是" : "否")")
        let enabled = UserDefaults.standard.bool(forKey: "contact_enabled_\(contactId)")
        logInfo.append("• contact_enabled_\(contactId): \(enabled ? "是" : "否")")
        let pinned = UserDefaults.standard.string(forKey: "contact_pinned_\(contactId)")
        logInfo.append("• contact_pinned_\(contactId): \(pinned != nil ? "是" : "否")")
        let hidden = UserDefaults.standard.bool(forKey: "contact_hidden_\(contactId)")
        logInfo.append("• contact_hidden_\(contactId): \(hidden ? "是" : "否")")
        logInfo.append("")
        
        // 数据信息
        logInfo.append("💾 数据信息:")
        if let sessionsData = UserDefaults.standard.data(forKey: "AIChatSessions") {
            logInfo.append("• AIChatSessions: \(sessionsData.count) 字节")
        } else {
            logInfo.append("• AIChatSessions: 未设置")
        }
        
        if let historyData = UserDefaults.standard.data(forKey: "chat_history_\(contactId)") {
            logInfo.append("• chat_history_\(contactId): \(historyData.count) 字节")
        } else {
            logInfo.append("• chat_history_\(contactId): 未设置")
        }
        logInfo.append("")
        
        // 内存和磁盘信息
        logInfo.append("💻 系统资源:")
        logInfo.append("• 内存使用: \(getMemoryUsage()) MB")
        logInfo.append("• 磁盘空间: \(getDiskSpace()) GB")
        
        let alert = UIAlertController(
            title: "详细诊断日志",
            message: logInfo.joined(separator: "\n"),
            preferredStyle: .alert
        )
        
        alert.addAction(UIAlertAction(title: "复制到剪贴板", style: .default) { _ in
            UIPasteboard.general.string = logInfo.joined(separator: "\n")
        })
    ...
    private func configureDeepSeekAPI() {
        print("🔧 配置DeepSeek API密钥")
        
        // 设置DeepSeek API密钥
        UserDefaults.standard.set("sk-02ba2913283c427a84a49ba2a22cb2ff", forKey: "deepseek_API_KEY")
        
        // 设置网络超时配置
        UserDefaults.standard.set(30.0, forKey: "network_timeout") // 30秒超时
        UserDefaults.standard.set(30.0, forKey: "api_timeout")     // API请求30秒超时
        UserDefaults.standard.set(30.0, forKey: "request_timeout") // 通用请求30秒超时
        
        // 设置联系人状态
        UserDefaults.standard.set(true, forKey: "contact_enabled_deepseek")   // 启用DeepSeek
        UserDefaults.standard.set(false, forKey: "contact_pinned_deepseek")   // 不置顶
        UserDefaults.standard.set(false, forKey: "contact_hidden_deepseek")   // 不隐藏
    ...
    private func getDiskSpace() -> String {
        do {
            ...
            if let freeSize = attributes[.systemFreeSize] as? NSNumber {
                ...
                return String(format: "%.1f", freeSizeGB)
            }
        } catch {
        ...
        VStack(spacing: 0) {
            ...
            List(filteredContacts, id: \.id) { contact in
                ContactRow(
                    ...
                    onTest: {
                        startAIChatTest(for: contact.id)
                    },
                    onClear: {
                        clearAIData(for: contact.id)
                    }
                )
            }
    ...
    var body: some View {
        ...
        .contextMenu {
            ...
            Button(action: onTest) {
                Label("测试初始化", systemImage: "wrench.and.screwdriver")
            }
            
            Button(action: onClear) {
                Label("清理AI数据", systemImage: "trash.fill")
            }
        }
    }
...
class SimpleContactsManager: ObservableObject {
    ...
    func loadContactSettings() {
        ...
        do {
            ...
            print("⚠️ SimpleContactsManager加载失败: \(error.localizedDescription)")
            // 重置为默认状态
            let defaultEnabled = Set([
                "deepseek", "qwen", "chatglm", "moonshot", "openai", "claude", "gemini"
            ])
    ...
    func saveContactSettings() {
        ...
        do {
            ...
            print("⚠️ SimpleContactsManager保存失败: \(error.localizedDescription)")
        }
    }
